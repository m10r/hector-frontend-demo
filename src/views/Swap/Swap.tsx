  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        setTrade(newTrade);
        setLoading(false);
        setFromUsd(prettyDisplayNumber(newFromUsd.multipliedBy(newTrade.from.tokenAmount)));
        setToUsd(prettyDisplayNumber(newToUsd.multipliedBy(newTrade.to.tokenAmount)));
      } catch (e) {
        if (disposed) {
          return;
        }
